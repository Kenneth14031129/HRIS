...
interface EditPayrollProps {
  isOpen: boolean
  onClose: () => void
  onUpdate: (payrollRecord: any) => void
  payrollRecord: any
}

const EditPayroll = ({ isOpen, onClose, onUpdate, payrollRecord }: EditPayrollProps) => {
  ...
  useEffect(() => {
    if (payrollRecord) {
      // Parse the pay period to get start and end dates
      const payPeriodParts = payrollRecord.payPeriod?.split(' to ') || ['', '']
      ...
      setFormData({
        ...
        payPeriodStart: payPeriodParts[0] || '',
        payPeriodEnd: payPeriodParts[1] || '',
        payDate: payrollRecord.payDate || '',
        baseSalary: payrollRecord.baseSalary?.toString() || '',
        overtime: payrollRecord.overtime?.toString() || '0',
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-700">
          <div>
            <h2 className="text-xl font-semibold text-white">Edit Payroll Record</h2>
            <p className="text-sm text-gray-400 mt-1">Update payroll information for {payrollRecord.employeeName}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {(formData.baseSalary || formData.overtime || formData.bonus || formData.deductions) && (
            <div className="bg-gradient-to-r from-yellow-500/10 to-yellow-600/5 border border-yellow-500/20 rounded-xl p-4">
              <h4 className="text-lg font-semibold text-white mb-3">Updated Pay Summary</h4>
